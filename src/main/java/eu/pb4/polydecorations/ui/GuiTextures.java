...
public class GuiTextures {
    public static final GuiElementBuilderCreator<?> EMPTY = icon16("empty").get();
    ...
    public record Progress(GuiElementBuilderCreator[] elements) {
        ...
        public static Progress createVertical(String path, int start, int stop, boolean reverse) {
            var size = stop - start;
            var elements = new GuiElementBuilderCreator[size + 1];
            var function = verticalProgress16(path, start, stop, reverse);

            elements[0] = EMPTY;

            for (var i = 1; i <= size; i++) {
                elements[i] = function.apply(i - 1);
            }
